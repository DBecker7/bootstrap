...
# Calculating a Bootstrap CI

1. Quantiles of the bootstrap sampling distribution

2. Estimate the standard error from the bootstrap sampling distribution
...
- Is not great for the median (and other quantiles).
  - What does this mean for method 1?

- Is ??? for skewed distributions (e.g. exponential).
...
- Bootstrap for percentiles 
  - I.e. why we usually use the standard error rather than 95% quartiles
...
:::{.notes}
In the next lesson, we'll investigate exactly why the bootstrap method is imperfect fro quartiles.
Spoilers: A plug in estimate for the whole sample isn't as important when the median is only calculated from the middle value or middle two values.

We'll also look at the bootstrap method for hypothesis testing, in particular how the t-statistic is *better* because it incorporates the covariance between the mean and the standard deviation.

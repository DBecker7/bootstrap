...
theta, covs = curve_fit(f = logistic_growth, 
    xdata = omicron.date, 
    ydata = omicron.prop, 
    p0 = [0.3, 15])
```
...
for i in range(midpoint_tracker.shape[0]):
    ...
    axis[0, 0].plot(xseq, yi, color = "grey", alpha = 0.01)
...
- Mean of bootstrap is (close to) mean of *sample*, not the population
...
Bootstrap estimates the sampling distr.
...
Bootstrap estimates the standard error.
...
Bootstrap emulates population, but median uses 1 or 2 points. This has implications for the "Quantiles" CI estimate.
:::
